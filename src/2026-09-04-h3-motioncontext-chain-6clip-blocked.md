...
Confirmed via install timestamps: the motion-context node package this whole chain depends on was installed weeks earlier; a separate attention-stack package, installed later for the unrelated monolithic 30-second attempts, patches the exact same interior layout restriction. Whichever package loads second refuses to double-patch rather than silently produce joins neither one intended, and the motion-context package is the one refusing here.
