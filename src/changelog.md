...
- [A VACE guide will not change its subject, so the same-subject question stays open](2026-09-24-h3-same-subject-guide.html) — testing whether an aligned guide transfers action needs a guide depicting the prompt's own subject, and Wan2.1 VACE at strength 1.0 will not produce one: it reproduces the source clip's suited man and ignores a prompt asking for someone else. Both guides are the same person, so the factor never varies and none of the twenty conditioned arms rendered.
